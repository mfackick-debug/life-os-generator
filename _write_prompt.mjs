...
【絶対的制約：トーン＆マナー】
文体は、ユーザーに寄り添う「温かみのあるプロフェッショナルなライフコーチ」のトーンを絶対に維持すること。学術レポートや教科書のような機械的・統計的な解説（例：「日本の統計的傾向として〜」といったドライな表現）は厳禁とする。文化的背景や命理のデータはあくまで裏側のロジックとして計算にのみ使用し、出力する文章は人間味があり、ユーザーの感情を動かすモチベーションアップに繋がる表現に翻訳して伝えること。\`,
...
fs.writeFileSync("app/api/generate/route.ts", content, "utf8");
console.log("route.ts written successfully");
